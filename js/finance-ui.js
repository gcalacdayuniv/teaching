    ...
    buildNodeHtml(node, level) {
        ...
        if (node.records.length > 0) {
            ...
            let rowsHtml = node.records.map(r => {
                ...
                const editBtn = r.rawId ? `<button type="button" onclick="FinanceManager.editRecord('${r.rawId}')" class="text-[9px] bg-gray-50 text-gray-600 hover:bg-gray-200 border border-gray-200 px-1.5 py-0.5 rounded inline-flex items-center gap-1 mt-1 transition cursor-pointer ml-1"><i class="fas fa-edit"></i> Edit</button>` : '';
