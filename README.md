...
* **`index.html` & `styles.css`:** Main entry point, the static layout shell, bottom nav, and FAB. `index.html` loads the app via `<script type="module" src="./js/app.js"></script>`. Includes navigations for Records, Finance, and Resources.
...
* **`js/finance.js`:** Manages the Finance Tracker module with a modernized responsive UI. Features a dynamic 6-level deep collapsible tree (Projects -> Main Group -> Sub-Group 1 to 5). Auto-defaults to the current month with global date-range filtering. Intelligently sorts Ledgers with Income categories first, Expenses second, and active Projects at the bottom (sorted by latest transaction, automatically hiding zero-value projects). Supports project creation and transaction logging with HTML5 Canvas Base64 receipt compression. Merges Teaching Earnings natively by aggregating payment dates and auto-categorizing under `Teaching -> University -> College/Department -> Subject`.
...
* **`Projects`:** Project_ID (UUID), Name, Created_At.
* **`Finance_Transactions`:** Transaction_ID (UUID), Date, Type, Main_Group, Sub_Group_1, Sub_Group_2, Sub_Group_3, Sub_Group_4, Sub_Group_5, Description, Amount, Project_ID, Attachment (Base64).
